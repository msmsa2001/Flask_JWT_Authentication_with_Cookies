...
def token_required(f):
    ...
    def decorated(*args,**kwargs):
        ...
        print("tokenasdfgdsdfghjsdfghjfghjdfg",token)
        ...
        try:
            ...
            print("data['public_id']",data['public_id'])
            if session['user_id']!=data['public_id']:
...
def login():
    ...
    if check_password_hash(user.password,auth.password):
        ...
        response = make_response(jsonify({'message': 'Token set in the cookie'}))
        session['user_id']=user.public_id
        response.set_cookie('current_user', token)
        return response
